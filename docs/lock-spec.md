...
The lock file is a resolved, per-user artifact produced by `sx install`.
It enumerates the exact assets, versions, sources, and content hashes the
calling user must install — with team and user install scopes already
flattened against the caller's identity — so downstream tooling has a
deterministic, identity-independent input. Inspired by PEP 751, it
prioritizes reproducibility and clear dependency tracking.

The source of truth is `sx.toml` at the vault root (see
[manifest-spec.md](manifest-spec.md)); the lock file here is derived from
it on every install.

## File location and rotation

Lock files live in the user's sx cache directory, not in the vault or the
project working tree:

```
~/.<cache-dir>/sx/lockfiles/
  <vault-id>.lock                         ← active lock
  <vault-id>-YYYY-MM-DDTHH-MM-SSZ.lock    ← rotated prior versions
```

`<vault-id>` is a stable hash of the vault URL. On every install the new
resolved lock is compared with the existing active file; if the content
...
# This file is a resolved artifact produced from sx.toml and
# the calling user's identity. Run `sx install` to regenerate.
```
...
- `ref`: Git reference to checkout (required)
  ...
  - Branch/tag names are resolved to commit SHAs during lock generation
...
### Use Case 1: Self-hosted file vault

1. A team maintains a path or git vault with an `sx.toml` manifest.
2. Each user runs `sx install`; the client reads the vault's manifest,
   resolves team/user scopes against the caller's git identity, and
   writes the resulting lock file to the per-user cache.
3. Clients install assets from the lock file's source specifications.

### Use Case 2: Server-Managed Lock File

1. User authenticates with a vault server (e.g. skills.new).
2. Server resolves the user's effective lock file and returns it over
   HTTP; the client caches it in the same per-user lock directory.
3. Server can update assets centrally by changing server-side state;
   `sx install` refreshes the client's cached lock.
