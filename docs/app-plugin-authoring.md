...
|---|---|
| — | `sx.ui.notice(msg)`, `sx.ui.confirm(msg, action)`, `sx.ui.openAsset(name, opts?)` (API 1.1.0 — opens an asset's detail panel, for making result rows navigable; `opts.tab` (API 1.11.0) opens one of your own registered asset tabs by id instead of the content view), `sx.ui.openSettings(section?)` (API 1.9.0 — open app Settings on `"libraries"`/`"extensions"`/`"ai"`; the deep link to show when your feature needs app-level setup), `sx.storage.loadData()/saveData(data)` (per user, per profile, local — up to 10 MB, room for an incremental event cache), `sx.app.version`, `sx.app.currentUser()` (API 1.5.0 — the identity vault changes are attributed to), `sx.api.version` |
...
| `views:team` | `sx.registerTeamView({id, title, mount})` (API 1.7.0) — a tab on a team's view, same contract as collection views. `mount(view, ctx)` receives `ctx.team`, the team name. Combine with `sx.teams.list()` (usage:read) for membership. |
| `views:repo` | `sx.registerRepoView({id, title, mount})` (API 1.7.0) — a tab on a repository's view, same contract. `mount(view, ctx)` receives `ctx.repo`, the repository URL. `sx.repos.list()` (assets:read) maps repo URL → asset names scoped there. |
| `export` | `sx.collections.export(name, format)` (API 1.6.0) — bundles the collection's assets into one file behind a native save dialog; resolves to the saved path, or `""` if the user cancels. `format`: `"zip"` (every asset, one folder each), `"claude-code"`, `"codex"`, or `"gemini"` (plugin bundles — **skill assets only**; other types are skipped). |
...
| `benchmarks` | `sx.benchmarks.list(asset)` / `add(asset, record)` / `latest()` (API 1.10.0) — per-asset benchmark records, unified across vault types: `.sx/benchmarks/<asset>.json` on file vaults, real benchmark rows on skills.new (so `list` includes server-run benchmarks, `source: "server"`). Newest first, capped per asset; `latest()` is the one-call dashboard read. See [benchmarks-spec.md](benchmarks-spec.md). |
...
| `assets:consolidate` | `sx.assets.consolidate({into, from})` (API 1.9.0) — move every `from` asset's installations onto `into`, then retire the `from` assets (removed from the library, recoverable from version history). The dangerous grant: always `sx.ui.confirm` first, name every asset in the prompt, and surface `skipped` (RBAC-refused moves). `sx.assets.installations(name)` (assets:read) shows an asset's reach. |
| `llm:use` | `sx.llm.complete({messages, schema?, model?, maxTokens?})` (API 1.9.0) — one completion against **the provider the user configured** in Settings → AI provider (an installed claude/codex/gemini CLI, a local Ollama model, or their own API key for Anthropic/OpenAI-compatible/Google endpoints). Returns `{text, json?, provider, model, usage}`; pass `schema` (JSON Schema) to get a validated JSON reply with `json` pre-parsed. Never assume a vendor, a model, or latency — CLI and local providers can take minutes on big prompts. `sx.llm.provider()` returns the configured provider id (`""` when none — show a "configure a provider in Settings" hint instead of calling). |
